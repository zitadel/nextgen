...
## I want to contribute to the CLI

For changes to `@zitadel/cli` (`apps/cli`) — the `zitadel` npm CLI that sets up,
syncs and runs a customer's local project.

| I want to...                                  | Run                                                              |
|-----------------------------------------------|------------------------------------------------------------------|
| Install dependencies                          | `corepack pnpm install --frozen-lockfile`                        |
| Build the CLI (and its package deps)          | `moon run cli:build`                                             |
| Run my build directly                         | `node apps/cli/bin/run.js <command>`                             |
| Run it the way a user does (local packages)   | `moon run workspace:cli -- <command>`                            |
| Run the unit and integration tests            | `moon run cli:test`                                              |
| Lint                                          | `moon run cli:lint`                                              |
| Regenerate the command reference              | `moon run cli:readme`                                            |
| Run the fresh-app journey                     | `moon run cli-journey-e2e:e2e-local`                             |

The CLI is an [oclif](https://oclif.io) CLI: ESM, Node ≥ 24, bundled with
`tsdown`. Commands live in `apps/cli/src/commands/`; the larger half of the code
is `apps/cli/src/lib/` — the sync engine (`lib/sync/`), framework scaffolding
(`lib/orca/`), and `lib/oclif/base.ts`, the `BaseCommand` every command extends
for global flags, log levels, the JSON envelope and telemetry.

Two related documents are part of the CLI's surface, not background reading:
[`apps/cli/SKILLS.md`](apps/cli/SKILLS.md) is the consumer-agent contract and
must be updated whenever CLI behavior changes, and
[`apps/cli/AGENTS.md`](apps/cli/AGENTS.md) holds the telemetry rules. The
command reference in `apps/cli/README.md` is generated by `oclif readme` — edit
the commands, not the README.

### Running the CLI from source

`bin/run.js` loads `dist/`, not `src/`, and oclif builds its command table from
`dist/index.mjs`. So **rebuild after every change**: a source edit that seems to
have no effect, or a new command reported as unknown, is almost always a stale
build.

```sh
moon run cli:build
node apps/cli/bin/run.js doctor --cwd /path/to/app
```

That is the fast path, and enough for anything that does not install packages
into a generated app.
...
This is the only way to exercise the parts of `setup` that install `@zitadel/*`
packages, so the generated app gets your SDK changes rather than published ones.
It is correspondingly slow — prefer the direct path above when you can.

### Tests

`apps/cli/tests/unit/` tests `src/lib/` directly and needs no build. Most new
tests belong there.

`apps/cli/tests/integration/` drives the **built** CLI in-process through
`runCliForTest` (`tests/helpers/run-cli.ts`), capturing stdout, stderr and the
exit code. `tests/helpers/global-setup.ts` builds before any test file runs, so
`vitest run` is self-contained but never faster than a build. Pass
`--non-interactive --json` and `ZITADEL_TELEMETRY: "0"` the way an agent would.

`tests/integration/contract.test.ts` enforces the JSON envelope across commands:
parseable JSON on stdout with top-level `cli_version`, `command`, `source` and
`status`, and no stray stdout text. If you add a command, that is the test that
will tell you the envelope is wrong.

> `cli:typecheck` currently checks nothing — it is marked `KNOWN-VACUOUS` in
> `apps/cli/moon.yml` because it loads a solution-style tsconfig with plain
> `tsc --noEmit`. Do not read a green typecheck there as type safety.

### Debugging the CLI

`--debug` raises logging to consola level 4 and is the first thing to try.
`--json` silences consola entirely so the envelope is the only thing on stdout,
which means `--json` and `--debug` fight each other by design — run the command
twice when you need both. `--verbose` exists but currently maps to the same
level as the default, so it changes nothing today.

`bin/run.js` is an ordinary Node entry point, so the inspector works:

```sh
node --inspect-brk apps/cli/bin/run.js setup --cwd /path/to/app
```

Attach from VSCode with `"request": "attach"`, or `chrome://inspect`. You are
stepping through bundled `dist/` output; `tsdown` emits source maps so
breakpoints in `src/` resolve, but only against the build you last made. A
breakpoint that never hits usually means a stale `dist/`.

To step through a test instead:

```sh
cd apps/cli && corepack pnpm exec vitest --inspect-brk --no-file-parallelism tests/integration/setup-next.test.ts
```

When `setup` succeeds but the generated app misbehaves, the bug is usually in
the scaffolding (`src/lib/orca/`) rather than the command. Generate into a
directory you keep and inspect it; `zitadel doctor --json` inside it reports
what the CLI believes it wrote against what is actually there, which separates
"wrote the wrong thing" from "wrote nothing". For server-side questions, run
`moon run workspace:server` in one terminal and point the CLI at it with
`--server http://localhost:8080` in another, so server logs and CLI output stay
separate — `--json` suppresses CLI logging exactly when you most want to
correlate them.

| Symptom                                       | Usually                                                                                           |
|-----------------------------------------------|---------------------------------------------------------------------------------------------------|
| A source change has no effect                 | Stale `dist/`. Rebuild.                                                                           |
| A new command is "not found"                  | Same — the command table lives in `dist/index.mjs`.                                               |
| `Cannot find module @zitadel/api/...`         | A dependency package's `dist/` is missing or stale; `cli:build` builds them.                      |
| Integration tests fail after a dep change     | A `build-release` task rewrote a shared `dist/` mid-run; the `public-dist` mutex exists for this. |
| The generated app installs published packages | The wrapper was bypassed, or `ZITADEL_CLI_USE_PUBLIC_PACKAGES` is set.                            |
| Telemetry fires while testing                 | Expected — local builds route to the dev Mixpanel project. Use `--no-telemetry`.                  |

### Before you open a CLI pull request

Beyond the repository-wide checks, a CLI change usually touches more than the
command file:

- update [`apps/cli/SKILLS.md`](apps/cli/SKILLS.md) if anything an agent can
  observe changed — flags, envelope fields, exit codes, command semantics;
- run `moon run cli:readme` if flags or descriptions moved;
- add a changeset listing `@zitadel/cli`;
- run `moon run cli-journey-e2e:e2e-local` if you touched setup, scaffolding or
  posture.
