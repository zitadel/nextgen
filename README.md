...
To run the integration tests against a database you manage instead of testcontainers, set `ZITADEL_TEST_POSTGRES_URL` (Postgres DSN) or `ZITADEL_TEST_SPANNER_URL` (Spanner DSN). The suites that read these connect to it instead of starting a container; a few suites (e.g. `internal/bootstrap/users` on Postgres and the Spanner API integration setup) always start their own container, so a Docker daemon is still required.
