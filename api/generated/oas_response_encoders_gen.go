...
func encodeGetIdpBySlugResponse(response GetIdpBySlugRes, w http.ResponseWriter, span trace.Span) error {
	switch response := response.(type) {
	case *IdpResponse:
		if err := func() error {
			if err := response.Validate(); err != nil {
	...
	case *GetIdpBySlugErrorResponseStatusCode:
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		code := response.StatusCode
		if code == 0 {
			// Set default status code.
