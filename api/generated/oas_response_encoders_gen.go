...
func encodeCreateEnvironmentResponse(response CreateEnvironmentRes, w http.ResponseWriter, span trace.Span) error {
	...
	case *CreateEnvironmentErrorResponseStatusCode:
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		code := response.StatusCode
		if code == 0 {
			// Set default status code.
...
func encodeCreateFlowResponse(response CreateFlowRes, w http.ResponseWriter, span trace.Span) error {
	...
	case *FlowResponseHeaders:
		...
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		w.Header().Set("Access-Control-Expose-Headers", "Set-Cookie,X-Zitadel-Environment,X-Zitadel-Release")
		// Encoding response headers.
		{
			...
			// Encode "X-Zitadel-Environment" header.
			{
				cfg := uri.HeaderParameterEncodingConfig{
				...
				if err := h.EncodeParam(cfg, func(e uri.Encoder) error {
					if val, ok := response.XZitadelEnvironment.Get(); ok {
						return e.EncodeValue(conv.StringToString(val))
					}
					return nil
				}); err != nil {
					return errors.Wrap(err, "encode X-Zitadel-Environment header")
				}
			}
			// Encode "X-Zitadel-Release" header.
			{
				cfg := uri.HeaderParameterEncodingConfig{
				...
				if err := h.EncodeParam(cfg, func(e uri.Encoder) error {
					if val, ok := response.XZitadelRelease.Get(); ok {
						return e.EncodeValue(conv.StringToString(val))
					}
					return nil
				}); err != nil {
					return errors.Wrap(err, "encode X-Zitadel-Release header")
				}
			}
		}
		w.WriteHeader(201)
