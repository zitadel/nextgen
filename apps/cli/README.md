...
- `zitadel plan` — preview local config changes without mutating files or the platform
- `zitadel apply` — validate and upload repo config to the platform
- `zitadel claim` — claim an anonymous local project into a team
...
USAGE
  $ zitadel claim [--json] [-c <value>] [-s <value>] [-n] [-f] [--dry-run] [--verbose] [--debug]
    [--challenge-id <value>] [--return-url <value>] [--team-name <value>]
...
FLAGS
  ...
  -n, --non-interactive       Disable prompts. Required when scripting or running as an agent.
  -s, --server=<value>        Override the resolved server URL.
      --challenge-id=<value>  Poll a claim challenge and rewrite .zitadel/secret after completion.
      --debug                 Debug logging.
      --dry-run               Preview without mutating files or the platform.
      --return-url=<value>    URL the browser claim flow should return to.
      --team-name=<value>     Suggested team name for the browser claim flow.
      --verbose               Verbose logging.
