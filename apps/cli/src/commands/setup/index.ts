...
export default class Setup extends BaseCommand {
  ...
  async run(): Promise<JsonEnvelope> {
    ...
    // `POST /projects` is unauthenticated; the returned `project_secret`
    ...
    const project = dryRun
      ? dryRunProject()
      : await unauthClient.createProject({ preview_origins: [] });
    ...
    if (!flags["no-apply"] && !dryRun) {
      ...
      await client.applyProjectConfig(secret.project_id, { environment: "development" }, { source: "cli" });
      apply = { synced: true };
    }
    ...
    return this.emit({
      ...
      data: {
        ...
        project: { project_id: project.project_id, lifecycle: project.lifecycle, issuer },
