...
export default class Setup extends BaseCommand {
  ...
  async run(): Promise<JsonEnvelope> {
    ...
    // SPIKE: on a CLI-managed local server the developer already exists as the
    // local admin, so the project is attached to their team right away and
    // `zitadel claim` has nothing left to do ("already owned").
