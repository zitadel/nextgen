 ...
 *
 * Preflights the project secret and any `${VAR}` / `*_env` references in the
 * flows before contacting the platform so missing configuration fails fast with
 * `E_VALIDATION`. In plan/dry-run mode it renders the diff and returns without
 * writing; otherwise it runs the sync loop to convergence.
 */
 ...
 * sorted and de-duplicated. Recognises two reference styles: inline `${VAR}`
 * interpolations inside string values, and keys ending in `_env` whose value
 * names a single variable. Used by {@link runApply} to fail before applying
