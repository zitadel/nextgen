...
Integrates Zitadel auth into a Vite + Svelte single-page app.

## What it patches

- `src/App.svelte` — auth entry that renders the Zitadel login/logout widgets
...
- `package.json` — adds `@zitadel/sdk-svelte`
...
`ZITADEL_PROJECT_SECRET`) to every proxied request. The production story is a
platform rewrite plus the publishable key from ADR 036
