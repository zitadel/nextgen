    ...
    return [
      ...
      { kind: "append-gitignore", entries: [".zitadel/secret", ".env*", "!.env.example"] },
      {
        ...
        contents: `${stableStringify({
          ...
          lifecycle: ctx.project.lifecycle,
      ...
      },
      {
        ...
        path: ".env.local",
        entries: {
          ZITADEL_PROJECT_ID: ctx.project.project_id,
          ZITADEL_ENVIRONMENT: "development",
          ZITADEL_ISSUER: ctx.issuer,
          NEXTGEN_ISSUER_URL: ctx.server,
          NEXT_PUBLIC_ZITADEL_PROJECT_ID: ctx.project.project_id,
        },
      },
      {
        kind: "write",
...
function projectConfig(ctx: PatchContext): Record<string, unknown> {
  ...
  if (ctx.project.preview_origins.length > 0) {
    environments.preview = {
      issuer_pattern: ctx.project.preview_origins.map((origin) => `https://${origin}`),
    };
  }
  return {
