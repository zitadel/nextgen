...
/**
 * SPIKE: the local admin a CLI-managed server is started with, so a developer
 * "exists" on their own server without signing up (Slack proposal, Sep 17).
 *
...
const FALLBACK_EMAIL = "admin@zitadel.localhost";
...
export async function ensureLocalAdmin(
  ...
  if (!admin) {
    admin = {
      email: (await gitEmail(cwd)) ?? FALLBACK_EMAIL,
...
async function gitEmail(cwd: string): Promise<string | undefined> {
  try {
    const { stdout } = await promisify(execFile)("git", ["config", "--get", "user.email"], { cwd });
