...
function toEnvelope(result: CommandResult, meta: GlobalOptions): JsonEnvelope {
  ...
  if (result.status === "ok") {
    return {
      ...
      next_commands: nextCommandsFromData(result.data),
    };
  }
  return {
    ...base,
...
function nextCommandsFromData(data: unknown): string[] | undefined {
  if (!isObject(data) || !Array.isArray(data.next_commands)) {
    return undefined;
  }
  const commands = data.next_commands.filter((cmd): cmd is string => typeof cmd === "string");
