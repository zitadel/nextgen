...
const oidcBlock = z.object({
  ...
  auth_methods: z.array(z.enum(["client_secret_basic", "client_secret_post", "none", "private_key_jwt"])).default(["client_secret_basic"]),
  grant_types: z.array(z.enum(["authorization_code", "refresh_token", "client_credentials", "urn:ietf:params:oauth:grant-type:token-exchange"])).default(["authorization_code", "refresh_token"]),
...
const samlBlock = z.object({
  ...
  (value) => Boolean(value.metadata_url || value.metadata_xml || value.acs_url),
