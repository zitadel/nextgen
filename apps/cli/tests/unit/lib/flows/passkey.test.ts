...
describe("passkey.build", () => {
  it("returns the passkey id", () => {
    expect(passkey.id).toBe("passkey");
  });

  it("emits a flow that round-trips through flowDefinitionSchema", () => {
    const { flow } = passkey.build({ fields: ["email", "given_name"] });
  ...
  it("emits a credential step with no fields and no forgot action", () => {
    const { flow } = passkey.build({ fields: ["email"] });
  ...
  it("does not seed credential.field.password or credential.action.forgot", () => {
    const { locale } = passkey.build({ fields: [] });
  ...
  it("includes register-step locale entries for the requested fields", () => {
    const { locale } = passkey.build({ fields: ["email", "given_name"] });
  ...
  it("returns freshly allocated objects on every call", () => {
    const a = passkey.build({ fields: ["email"] });
    const b = passkey.build({ fields: ["email"] });
