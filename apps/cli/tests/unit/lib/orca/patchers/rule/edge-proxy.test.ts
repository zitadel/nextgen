...
describe("edgeProxyOps — vercel", () => {
	...
	it("writes a managed root middleware that matches /__nextgen directly", () => {
	...
	it("does not write a vercel.json or an api/ function (middleware intercepts directly)", () => {
		expect(editOp(ops, "vercel.json")).toBeUndefined();
		expect(writeOp(ops, "api/__nextgen/[...path].ts")).toBeUndefined();
	});
