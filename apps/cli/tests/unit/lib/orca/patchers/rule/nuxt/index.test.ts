...
describe("NuxtPatcher.plan", () => {
  it("writes the managed app + pages and edits nuxt.config.ts", () => {
...
describe("NuxtPatcher.artifacts", () => {
  it("lists the pages as managed and nuxt.config.ts as a manual config edit", () => {
    const artifacts = new NuxtPatcher().artifacts({
      framework: { id: "nuxt", appDir: ".", devPort: 3000, url: "http://localhost:3000" },
      rendererId: "react",
    });
    expect(artifacts.markedFiles).toContain("pages/login.vue");
