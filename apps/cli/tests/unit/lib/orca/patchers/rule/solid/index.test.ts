...
function ctx(): PatchContext {
  return {
    framework: { id: "solid", appDir: "src", devPort: 3000, url: "http://localhost:3000" },
    rendererId: "solid",
    project: {
      id: "proj-1",
...
describe("SolidPatcher.plan", () => {
  ...
  it("wires the business copy overlay for business-use-case projects", () => {
    ...
    expect(writeContents(new SolidPatcher().plan(ctx()), "src/App.tsx")).not.toContain(
      "businessLocales",
    );
    const business = writeContents(
      new SolidPatcher().plan({ ...ctx(), useCase: "business" }),
      "src/App.tsx",
    );
    ...
    const consumer = new SolidPatcher().plan({ ...ctx(), useCase: "consumer" });
...
describe("SolidPatcher.artifacts", () => {
  it("lists App.tsx as managed and the Vite config as a manual config edit", () => {
    const artifacts = new SolidPatcher().artifacts({
      framework: { id: "solid", appDir: "src", devPort: 3000, url: "http://localhost:3000" },
      rendererId: "solid",
    });
    expect(artifacts.markedFiles).toContain("src/App.tsx");
    expect(artifacts.dependencies).toEqual(["@zitadel/sdk-solid"]);
