...
describe("status command", () => {
  it("returns an ok envelope for a healthy project with config and secret", async () => {
    ...
    expect(json.data.next_commands).toContain("npx @zitadel/cli@preview doctor");
  });
  ...
  it("reports orphaned-config when zitadel.json exists but secret is missing", async () => {
    ...
    expect(json.status).toBe("ok");
    expect(json.data.next_commands).toContain("npx @zitadel/cli@preview setup --force");
  ...
  it("reports not-configured when zitadel.json is missing entirely", async () => {
    ...
    expect(json.data.next_commands).toContain("npx @zitadel/cli@preview start");
    expect(json.data.next_commands).toContain("npx @zitadel/cli@preview setup --server local");
  });
