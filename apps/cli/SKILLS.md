...
- On failure: `code` (e.g. `E_VALIDATION`, `E_NETWORK`, `E_CONFLICT`,
  `E_CLAIM_REQUIRED`) and `message`.
...
6 = claim-required, 1 = auth, 2 = not-implemented). An unknown command is
handled by the CLI's help layer, not the envelope.
...
- `plan` — validate config and preview the sync diff without mutating anything.
  `--environment preview|production` requires a claimed project.
- `apply` — validate and upload repo config to the platform. Development is
  allowed before claim; preview and production return `E_CLAIM_REQUIRED` until
  `claim` completes.
- `claim` — start or poll the human claim flow. Use `--challenge-id` to poll a
  challenge returned by a previous non-interactive run. `--dry-run` reads local
  state only and does not call the API or rewrite `.zitadel/secret`.
...
Before preview or production, run `npx zitadel@latest claim --non-interactive
--json` and hand the returned `claim_url` to the human owner.
