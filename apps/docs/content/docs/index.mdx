...
`zitadel start` runs the released `@zitadel/server` npm binary by default. It does not require Docker, Go, Moon, or a source checkout. The binary runtime defaults to SQLite under the local data directory. Docker is still available explicitly with `zitadel start --runtime docker`.
...
- [Databases](/docs/concepts/databases) for SQLite (local default), PostgreSQL, and Spanner.
