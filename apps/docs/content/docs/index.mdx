...
Create-first, claim-later remains product direction. The `zitadel claim` CLI command and the OpenAPI claim endpoints are shipped, but the server-side claim handlers are not implemented yet, so the flow does not complete against a real server. Do not build automation or docs around the full claim flow until the server implementation lands.
