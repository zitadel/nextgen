---
title: Next-generation Zitadel preview
...
`zitadel start` runs the released `@zitadel/server` npm binary by default. It does not require Docker, Go, Moon, or a source checkout. Docker is still available explicitly with `zitadel start --runtime docker`.
...
- [Concepts](/docs/concepts) for project, team, user, local runtime, repo config, flows, schemas, SDK proxy, sessions, and auth attempts.
