...
- `claim` attaches a project to your team. On the default local runtime, setup already did that, so `claim` reports the project as already owned; `claim` is the fallback when that step failed.
