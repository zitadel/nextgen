...
The sidebar is the selected project's contents and nothing else. The Projects
overview is about no single project, so it is not a sidebar entry: the project
switcher's `All projects` footer links to it, and a row there opens the project
(selects it). The project's own page is the scoped `/project` (`Project
