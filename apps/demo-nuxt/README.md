...
**Login page** renders the `<nextgen-login>` web component inside `<ClientOnly>` to avoid SSR.

**Admin page** reads `useState('nextgen-auth')` to display the signed-in user's email, with the `<nextgen-logout>` component in the header.
