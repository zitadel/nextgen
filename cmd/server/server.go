...
func run(ctx context.Context, cfg Config, userFiles []string, applyMigrations bool) error {
	...
	mux, err := buildHTTPMux(cfg.Server, idgen.NewULID(), oasServer, apiHandler,
...
func buildHTTPMux(cfg ServerConfig, reqIdGen middleware.RequestIDGenerator, apiHandler http.Handler, handler *api.Handler, runtime runtimeResolver, requestEvents *audit.RequestBuffer) (*http.ServeMux, error) {
	mux := http.NewServeMux()

	// Stub: the identity provider's return leg, which the OpenAPI does not
	// model because the real engine owns it (#1032). Registered as an exact
	// path so it wins over the catch-all API mount below, and removed with
	// `internal/api/idp_sso_stub.go`.
	mux.Handle(api.IdpCallbackPath, handler.IdpCallbackHandler())
