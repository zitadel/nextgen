...
no longer treats the password box the browser posts alongside it as a
submission, so the WebAuthn prompt appears instead of a required-field or
invalid-credentials error. And going back to the identifier step now releases
the user resolved by the abandoned attempt, so re-entering an address signs in
normally instead of failing with "The user was already authenticated".
