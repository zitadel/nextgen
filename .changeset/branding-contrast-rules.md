...
`@zitadel/config/branding-contrast` names the pairs a login surface puts next to each other, the ratio each has to reach, and measures a palette against them — a shared rule for whatever comes to enforce or display it. Thresholds are WCAG 2.2 AA: 4.5:1 for text, 3:1 for the outline of a control. Nothing calls it yet.
