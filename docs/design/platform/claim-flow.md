...
> **Status:** Draft
> **See also:** [README](README.md) · [Overview](overview.md) · [Project Secret](nonce.md) · [Configuration Surface](configuration-surface.md) · [Claim API](api/claim-api.yaml)

Claim is the transaction that attaches ownership and accountability to a project. Before claim, the project exists but has no accountable owner. After claim, it belongs to an organization with at least one accountable human. The transition is atomic — nothing partial.

This document specifies what authenticates the claim, the sequence of operations, what changes as a result, how organizations are resolved, how failure modes are handled, and why agents cannot claim on behalf of humans.

## Claim is the accountability event

`project_id` has been stable since the server created it at `POST /v1/projects`. Users, factors, sessions, uploaded configs, declared issuers — everything has been bound to `project_id` from day one. What claim *does* is attach an organization (and a human, and a billing relationship, and a recovery path) to that project. It does not move identity or rewrite resource bindings.
...
sequenceDiagram
    ...
    CLI->>Secret: read project_id + current zp_
    CLI->>Srv: POST /v1/projects/{id}/claim/init<br>(Bearer: current zp_)
    Srv-->>CLI: { claim_url, challenge_id, expires_at }
    CLI->>Browser: open claim_url

    loop until completed or challenge expires
      CLI->>Srv: GET /claim/status?challenge_id=...<br>(Bearer: current zp_)
    ...
    Browser->>Srv: POST /claim/{challenge_id}/complete<br>(verified identity, org_choice)

    Srv->>Srv: atomic transaction:<br>1. resolve/create organization<br>2. create membership<br>3. rotate zp_ project secret<br>4. stash rotated secret<br>   keyed by challenge_id<br>5. mark challenge completed
    Srv-->>Browser: success page → redirect to dashboard

    CLI->>Srv: GET /claim/status?challenge_id=...<br>(Bearer: current zp_)
    Srv-->>CLI: { status: "completed", new_project_secret,<br>organization_id, claimed_at, dashboard_url }
...
**Why the CLI polls instead of the browser handing the secret back.** The rotated project secret is a production-grade bearer — we do not want it traversing the browser (where a hostile script on the claim page, a malicious browser extension, or an accidentally-logged response body could exfiltrate it). The CLI already holds the pre-claim `zp_`, which is also the proof-of-possession the server uses to authorize retrieval of the rotated secret: only a poller presenting the exact bearer that initiated the challenge gets the rotated value, and only once. This is the OAuth 2.0 Device Authorization Grant shape (RFC 8628), adapted to a CLI that already has a secret.

The pre-claim `zp_` stays valid until the atomic transaction commits; the server erases the stashed rotated value after a single successful retrieval. If the CLI crashes between "transaction committed" and "wrote `.zitadel/secret`," recovery is `npx zitadel secret restore` (post-claim), which re-authenticates the human and refreshes the local file. See [Recovery](#recovery).

## The atomic transaction

All of the following happen in a single database transaction at the server:

1. **Resolve or create organization.** Match the verified email's domain against existing organizations (see [Organization resolution](#organization-resolution)). If a match is found and the human confirms, the project is attached to the existing org. If the human chooses "create new" or the domain is a public-email provider, a new org is created with the human as first owner.
2. **Create membership.** The authenticated human becomes a member of the resolved organization with owner role.
3. **Rotate the project secret.** The pre-claim `zp_…` secret is invalidated; a new `zp_…` bound to the claimed project + organization is issued. The preview secret is untouched.
...
- **Ownership.** The project transitions from anonymous (no organization) to owned (attached to an organization with at least one member).
...
- **Project secret.** Rotated from the anonymous `zp_…` value to a new claimed credential. Everything else — preview secret, `project_id`, config, resources — untouched.
...
At claim completion, the server inspects the verified email's domain and attempts to match it to an existing organization.

### Domain matching with public-email suppression

If the domain is **not** a public email provider, the server checks for an existing organization whose primary verified domain matches. If found, the claim flow presents:

> Your email `florian@acme.com` matches the existing organization **Acme**.
> Join Acme with this project? *(recommended)*
> Or create a new organization.

**Default action: join.** This reverses the industry default of "create new unless told otherwise," and the reverse is correct because in the common case the developer wants to be in the same org as their colleagues.
...
> Claiming for a team? Enter your work email to route to your company's organization.
> Or continue with a personal workspace.

Without this guard, domain matching would group every random developer who claims via a GitHub account associated with a Gmail address into a single global "Gmail" organization. That failure mode is catastrophic and is blocked at the design level.
...
For CI scripts and deterministic agent-driven flows, the CLI accepts `--org acme`:

```
npx zitadel claim --org acme
```

This forces the claim into the specified organization. The authenticating human must already be a member of `acme`. This avoids surprise attachment while supporting automation.

### Invitation by owner

Organization owners invite members by email via the standard claim flow. The recipient accepts by claiming their own project, which is attached to the inviting organization instead of matching by domain. This is the fallback for employees on vanity domains, consultants, or anyone whose auto-matching does not work.

### Membership vs. transfer

Joining an organization with a project attaches the project to the organization. It does not transfer individual ownership of the human's account. Humans can be members of multiple organizations; projects belong to exactly one organization. This mirrors Linear, Vercel, and GitHub.

Post-hoc consolidation — transferring a project from one organization to another — is supported via a dedicated `project transfer` operation (not via claim). The owner of the source org must also be a member of the destination org. A 30-day undo window applies.

## Failure modes

### Duplicate claim attempt

If the project is already claimed when `POST /v1/projects/{id}/claim/init` is called, the server returns HTTP 409 with the existing dashboard URL and organization name in the response body:

```json
{
  "error": "already_claimed",
  "message": "This project is already claimed by organization Acme.",
  "organization_id": "org_acme",
  "dashboard_url": "https://dashboard.zitadel.cloud/org_acme/projects/river-8421"
}
```

The CLI prints a helpful message pointing the developer to their existing project and exits cleanly.

### Organization creation race

Two colleagues at the same company each run `npx zitadel claim` within a few seconds on separate projects. Both attempt to create a new organization for `acme.com`. The server resolves deterministically:

1. The transaction that commits first wins: it creates organization `org_acme`.
2. The losing transaction retries inside a short backoff window. On retry, it finds the organization, and the second project joins as an additional project.
3. An internal flag on the losing project records that the creation order was racy; this is surfaced to the organization owner's dashboard for manual review if desired.

No user-visible error occurs in the common case. The two projects end up in the same organization, which is almost always what the humans wanted.
...
Once claimed, a project has an accountable human (or multiple, via organization membership). Recovery is conventional:

1. **Lost access to primary auth provider** (e.g. GitHub account compromised). Recover via Google if linked at claim or later.
2. **Lost access to all linked providers.** Recover via the email address of any other organization member with sufficient role; they can invite the recovering human back.
3. **Lost access to the entire organization.** Support-assisted recovery using billing records, DNS ownership verification (for custom domains), and out-of-band identity checks. Documented SLA on the support channel.

### Pre-claim

Unclaimed projects **have no strong recovery guarantee**. See [Project Secret — Failure modes](nonce.md#failure-modes). The default CLI messaging emphasizes claiming early when the data matters.
...
These let agents perform preview deploys, E2E test runs, and similar automation with the owner clearly accountable. The token specification is out of scope for this document.

## See also

- [Project Secret](nonce.md) — what the project secret is and how it rotates at claim
