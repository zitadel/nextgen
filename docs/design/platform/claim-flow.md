...
sequenceDiagram
    ...
    CLI->>Srv: POST /projects/{project_id}/claim/init<br>(Bearer: current sk_proj_)
    Srv-->>CLI: { claim_url, challenge_id, expires_at }
    CLI->>Browser: open claim_url

    loop until completed or challenge expires
      CLI->>Srv: GET /projects/{project_id}/claim/status?challenge_id=...<br>(Bearer: current sk_proj_)
    ...
    Browser->>Srv: POST /projects/{project_id}/claim/complete<br>({ challenge_id, team_choice })
    ...
    CLI->>Srv: GET /projects/{project_id}/claim/status?challenge_id=...<br>(Bearer: current sk_proj_)
    Srv-->>CLI: { status: "completed", rotated_project_secret,<br>claimed_at }
    Srv->>Srv: erase stashed secret<br>(one-shot retrieval)
    CLI->>Secret: rewrite .zitadel/secret atomically
    CLI->>Dev: ✓ Claimed project, secret rotated
```
...
The pre-claim `sk_proj_` stays valid until the atomic transaction commits; the server erases the stashed rotated value after a single successful retrieval from `GET /projects/{project_id}/claim/status?challenge_id=...`. If the CLI crashes between "transaction committed" and "wrote `.zitadel/secret`," recovery will use a future post-claim restore flow that re-authenticates the human and refreshes the local file. See [Recovery](#recovery).
...
4. **Stash the rotated secret** server-side, keyed by `challenge_id` and bound to the hash of the pre-claim secret that initiated the challenge. The CLI retrieves it once via `GET /projects/{project_id}/claim/status?challenge_id=...`; the stashed value is erased on retrieval or at challenge expiry.
5. **Mark challenge completed.** The `challenge_id` from `POST /projects/{project_id}/claim/init` transitions to `completed`; subsequent completion attempts return 410.
...
For CI scripts and deterministic agent-driven flows, the CLI accepts `--team-name Acme` as a browser-facing suggestion:

```
npx zitadel claim --team-name Acme
```

The authenticating human still chooses or confirms the final team in the browser. This avoids surprise attachment while supporting automation.
...
If the project is already claimed when `POST /projects/{project_id}/claim/init` is called, the server returns HTTP 409 with the existing dashboard URL and team name in the response body:
...
- **Share preview or production deploys before claim.** `zitadel plan` and `zitadel apply` return a claim-required error for preview and production environments until a human claims the project. Deploy-platform secret handoff follows the same boundary when it ships.
...
**Agent-scoped credentials after claim.** Once claimed, future preview-scoped and CI-scoped tokens can let agents perform preview deploys, E2E test runs, and similar automation with the owner clearly accountable. The token specification is out of scope for this document; the canonical credential taxonomy lives in [`../api/credentials.md`](../api/credentials.md).
...
- [Claim API](api/claim-api.yaml) — HTTP surface for `/projects/{project_id}/claim/*`
