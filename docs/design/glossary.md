...
| **design** | Historical: a named Liquid starting point (`centered`, `split`, `split-right`, `hero`, `minimal`) that `zitadel branding eject --design` still scaffolds today. [ADR 056](../adrs/056-login-customization-categories.md) retires that catalog as the setup path — those files are page chrome, not widget structure. |
...
| **page chrome** | Everything outside `<zitadel-login>`: split page, modal host, left-pane marketing. Embedders write application code. Zitadel-served page chrome is **unset**. [ADR 056](../adrs/056-login-customization-categories.md). |
