...
| **page chrome** | Everything outside `<zitadel-login>`: split page, modal host, left-pane marketing. Authored as application code when the widget is embedded; as a hosted page-template setting when Zitadel serves the page. [ADR 056](../adrs/056-login-customization-categories.md). |
| **page template** | A catalog starting point for page chrome (centered, split-left, split-right, modal). Distinct from a **design**, which produces Liquid inside the widget. |
