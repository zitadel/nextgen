...
| **design** | A named starting point from the shipped catalog (`centered`, `split`, `split-right`, `hero`, `minimal`) that `zitadel branding eject --design` scaffolds. A design *produces* a widget template; once edited, what you own is a template, no longer a design. Same relationship as sign-in **preset** : flow definition. Not a page wrapper. |
