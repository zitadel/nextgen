...
* **State Storage Shape:** Fields on the attempt, or a dedicated table. Consumption must be atomic under concurrent duplicate callbacks (one succeeds, the second gets a reused-state error); `zitadel/zitadel` has no such guard ([`idp_intent.go#L169-L199`](https://github.com/zitadel/zitadel/blob/d488ecb07ffe82d1e5493e9482be48a3e82397cc/internal/command/idp_intent.go#L169-L199)) and no TTL on a pending state. Minting is unauthenticated, so the shape must make it cheap to bound pending records per flow and rate-limit minting.
