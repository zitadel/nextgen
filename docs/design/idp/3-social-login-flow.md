...
| **Flow Scaffolding:** Scaffold `sso_providers` on register-purpose steps and the conflict step with its login route. | CLI Journey (Area 4) |
...
* **Cross-Schema Subject Resolution:** Identity links are `(connection, subject)` pairings, which are schema-agnostic, but users belong to specific schemas and flows pin one schema revision. Behavior is currently undefined when a known subject belongs to a schema different from the active flow's pin.
