...
| **Identity-Critical Revision** | Warning | `issuer`, `subject_claim`, or an oauth2 `token_endpoint`/`userinfo_endpoint` changed on a state-tracked connection. The unique connection id keeps existing identity links, so they re-key to the new authority; a same-authority move (e.g. a custom-domain change) is legitimate, which is why this warns instead of erroring. The safe path for a real authority change is the rename path (see [Lifecycle](#connection-lifecycle)). |
