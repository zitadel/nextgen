...
| Revisioned on the server                                  | In-flight attempts, rollback, and audit need immutable revisions. The CLI syncer is the `mutable` kind, not `revisioned`: the server mints each revision under a stable lineage id ([area 4](4-cli-provider-setup.md#the-idpsyncer-component)) |
...
| **Identity-Critical Revision** | Warning | `issuer`, `subject_claim`, or an oauth2 `token_endpoint`/`userinfo_endpoint` changed on a state-tracked connection. The lineage id keeps existing identity links, so they re-key to the new authority; a same-authority move (e.g. a custom-domain change) is legitimate, which is why this warns instead of erroring. The safe path for a real authority change is the rename path (see [Lifecycle](#connection-lifecycle)). |
...
- **Catalog claims schemas:** Plain claim-name values are provider-controlled and unchecked at plan time. The catalog already carries per-vendor claim recipes, and strategy contracts declare their emitted claims, so promoting each template's recipe to a small claims schema would turn a typo'd `claim_mapping` value or `verified_claims` source into a plan-time warning for `google` and `github`, with custom providers keeping today's unchecked behavior. It is also the base for any future typed or expression mapping. Not 851 work.
