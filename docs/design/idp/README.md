...
| Provisioning and verification | • `creation` on the connection: `disabled` or `auto` (default)<br>• `verified_claims` evaluated per property at callback; the result is written to the server log, not enforced | • `creation: auto_only`<br>• `x-verify`<br>• gating creation on verification<br>• `is_auto_update`<br>• per-user-type narrowing through authentication method settings ([#898](https://github.com/zitadel/nextgen/issues/898))                                                                | [1](1-resource-model.md), [3](3-social-login-flow.md) |
