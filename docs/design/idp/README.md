...
| Provisioning and verification | • `is_creation_allowed`, `is_auto_creation`<br>• `verified_claims` evaluated per property at callback; the result is reported by the test journey, not enforced | • `x-verify`<br>• gating creation on verification<br>• `is_auto_update`                                                                | [1](1-resource-model.md), [3](3-social-login-flow.md) |
