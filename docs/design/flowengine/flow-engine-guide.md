...
## Security Model: Template Rendering and XSS

Rendering user-controlled or admin-controlled templates via `innerHTML` introduces a critical attack surface. The following documents the known vectors and the defense-in-depth mitigations.

### Attack Vector 1: Payload Injection (backend → template)
...
### Attack Vector 2: `innerHTML` and inline event handlers
...
### Attack Vector 3: Ejected custom templates (tenant admin)
...
### Attack Vector 4: Translation dictionary injection
...
### Attack Vector 5: The `| raw` filter bypass
