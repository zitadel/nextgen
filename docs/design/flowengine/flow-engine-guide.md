...
| Annotation | Effect |
|---|---|
| `x-unique: "project"` | Value must be unique at the given scope. A non-empty scope makes the field an identifier: the engine looks up the user on submit, which implies a `user_not_found` outcome in transitions. |
