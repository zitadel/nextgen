...
without AuthGate. `createFlow` mints `flow_id` before `CreateAuthAttempt` so
Path B `auth.attempt.created` shares `flow_id` / `session_id` columns with
Path A `request.api`. Probes (`healthz` / `readyz` / `livez`) do not.
