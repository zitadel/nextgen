...
| [016](016-global-api-initializer.md) | Global API Initializer | Replace implicit `setApiBaseUrl` singleton with explicit `initNextgen()` at app startup; framework SDKs auto-initialize from middleware config; aligns with Firebase `initializeApp()` pattern. |
