...
| [016](016-global-api-initializer.md) | Global SDK Configuration | Write-once `configureNextgen()` sets app-wide config (`apiBase`, `projectId`, `sessionExchangePath`) once at startup; web components read from shared config, attributes become optional overrides. |
