...
| [002](002-multi-package-release-strategy.md) | Multi-package Release Strategy | Moon owns the repo task graph and non-npm artifact builds; Changesets owns package versions, changelogs, npm publishing, and public package tags. |
...
| [022](022-flow-back-navigation.md) | Flow Back-Navigation | Back-navigation uses the existing action contract. The `<zitadel-login>` orchestrator additionally integrates with the browser's **History API** so that the native back gesture maps to the same `back` action without reloading the page or losing widget state. |
...
| [024](024-user-team-lifecycle-ownership.md) | User, Team, and Lifecycle Ownership | Users are project-scoped identities; each user has one lifecycle owner (`self` or `team`); team participation is separate from lifecycle ownership; delete operations deactivate/tombstone before purge. |
| [025](025-default-brand-font-loading.md) | Default Brand Font Loading | `@zitadel/components` loads the design-system brand face (Arimo) by default via a document-level stylesheet link (`applyDefaultFont`/`DEFAULT_BRAND_FONT_HREF`), so the auth UI paints the brand font even when the server returns no branding; tenant `branding.font_url` overrides it. Shipped default uses a linked stylesheet; self-hosting is a tracked follow-up. |
| [026](026-flow-action-kind.md) | Flow Action Kind | Every step action declares a `kind` (`submit`, `passkey`, `passkey_register`, `navigate`) to differentiate actions that process input and advance the state machine from navigation-only actions. |
