...
> **Status:** Withdrawn
> **Superseded by:** [ADR 041: Claim Lifecycle v2](041-claim-lifecycle-v2.md)
> **Date:** 2026-04-26
> **Context:** AI-native Zitadel CLI onboarding

## Withdrawal

The pre-claim / claim lifecycle has been removed from the CLI and api-mock pending a server-side `claim` contract (`/projects/{id}/claim/init` and `/projects/{id}/claim/status` are not in the OpenAPI spec). If and when the backend lands, a follow-up ADR will re-propose the lifecycle aligned with the shipped server surface. **That follow-up is now [ADR 041: Claim Lifecycle v2](041-claim-lifecycle-v2.md)**, which supersedes this ADR.
