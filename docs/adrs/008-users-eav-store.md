...
* **Hash Scoping:** The registry uses an `team_id` field that is populated with the actual ID for Team-scoped uniqueness or an empty string (`''`) for Global-scoped uniqueness, allowing a single table to handle both B2B and B2C constraints.
