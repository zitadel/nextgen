...
| Refresh Token               | Auth server     | Client | Opaque                               | Long: days/weeks (default 2 weeks)                                               | Because refresh tokens are long-lived, they need to be single-use and revocable. This is to mitigate replay attacks. Refresh tokens need to be exchanged for access-tokens. In the token response, an new refresh token is provided. |
| Personal Access Token (PAT) | Auth server     | Client | Opaque                               | Very long: months/years/infinite (default 3 months, infinite: not recommendable) | Because PATs are long-lived, they need to be revocable. PATs need to be exchanged for access-tokens.                                                                                                                                 |
...
Only tokens which are long-lived (refresh tokens and PATs) should be stored in
the server because the need to be able to be revoked. Access tokens and session
tokens should not be stored in the server.

The tokens table should be a minimal table. All data to link a token to another
resource, should live inside the token. Once a token is revoked, we can remove/
mark the token as inactive in the database which will make future validations 
fail.
