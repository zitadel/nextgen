...
2. **Managed** durable resources (users, teams, apps, IdPs) exposed through the API. HTTP create does **not** accept a client-chosen primary key; the dialect generates a prefixed string on insert (see [ADR 046](046-dialect-id-generation.md)).
...
We need one storage-layer story that works on PostgreSQL and Spanner without `interface{}` in every repository. Managed ID generation lives in the v2 dialect layer ([ADR 046](046-dialect-id-generation.md)), not domain.
...
Package-role ownership of generation is refined in [ADR 046](046-dialect-id-generation.md).
