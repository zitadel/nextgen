...
func TestUserTOTPStatements_CRUD(t *testing.T) {
	...
	require.NoError(t, s.CreateUser(ctx, newTestUser(t, projectID, schemaURL, userID, "totp@example.com", "TOTP User")))

	secret := []byte("totp-secret-bytes")
	require.NoError(t, s.CreateUserTOTP(ctx, &domain.CreateUserTOTP{
		ProjectID: projectID,
		UserID:    userID,
		Secret:    secret,
	}))
	...
	t.Cleanup(func() {
		_ = s.DeleteUserTOTP(context.Background(), byUser)
	})

	got, err := s.GetUserTOTP(ctx, byUser)
	...
	byID, err := s.GetUserTOTP(ctx, totpByIDFilter(got.ID))
	require.NoError(t, err)
	assert.Equal(t, got.ID, byID.ID)

	listed, err := s.ListUserTOTPs(ctx, &database.ListOptions[domain.UserTOTPField]{
	...
	got.Secret[0] ^= 0xff
	again, err := s.GetUserTOTP(ctx, byUser)
	require.NoError(t, err)
	assert.Equal(t, secret, again.Secret)

	require.NoError(t, s.DeleteUserTOTP(ctx, totpByIDFilter(got.ID)))
	_, err = s.GetUserTOTP(ctx, byUser)
	...
	got2, err := s.GetUserTOTP(ctx, byUser)
	require.NoError(t, err)
	require.Positive(t, got2.ID)
	require.NoError(t, s.DeleteUserTOTP(ctx, byUser))
	_, err = s.GetUserTOTP(ctx, byUser)
	assert.ErrorIs(t, err, new(database.NoRowFoundError))
}
...
func TestUserTOTPStatements_Update(t *testing.T) {
	...
	require.NoError(t, s.CreateUser(ctx, newTestUser(t, projectID, schemaURL, userID, "totp-upd@example.com", "TOTP Update")))
	require.NoError(t, s.CreateUserTOTP(ctx, &domain.CreateUserTOTP{
		ProjectID: projectID,
		UserID:    userID,
	...
	err := s.UpdateUserTOTP(ctx, byUser)
	assert.ErrorIs(t, err, database.ErrNoChanges)

	err = s.UpdateUserTOTP(ctx, totpByUserFilter(projectID, "missing-user"),
	...
	require.NoError(t, s.UpdateUserTOTP(ctx, byUser,
