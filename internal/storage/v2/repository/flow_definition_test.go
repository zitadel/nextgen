...
import (
	...
	"github.com/zitadel/nextgen/internal/storage/v2/repository"
)
...
func TestFlowDefinitionRepository_ListCursorPagination(t *testing.T) {
	tx, rollback := transactionForRollback(t)
	defer rollback()

	repo := repository.NewFlowDefinitionRepository(tx)

	for _, id := range []string{"flow-jc1", "flow-jc2", "flow-jc3", "flow-jc4"} {
		require.NoError(t, repo.CreateFlowDefinition(t.Context(), tx, sampleFlowDefinition("proj-jcursor", id)))
	}

	page1, err := repo.ListFlowDefinitionsPage(t.Context(), tx, "proj-jcursor",
		domain.WithFlowDefinitionLimit(2))
	require.NoError(t, err)
	require.Len(t, page1.Items, 2)
	require.NotNil(t, page1.NextCursor)

	page2, err := repo.ListFlowDefinitionsPage(t.Context(), tx, "proj-jcursor",
		domain.WithFlowDefinitionLimit(2),
		domain.WithFlowDefinitionCursor(*page1.NextCursor))
	require.NoError(t, err)
	require.Len(t, page2.Items, 2)
	assert.NotEqual(t, page1.Items[0].ID, page2.Items[0].ID)

	for _, id := range []string{page1.Items[0].ID, page1.Items[1].ID, page2.Items[0].ID, page2.Items[1].ID} {
