...
// Compiled holds generated SQL and positional arguments.
type Compiled struct {
	SQL  string
	Args []any
}

// ArgBag collects positional PostgreSQL arguments ($1, $2, ...).
type ArgBag struct {
	Args []any
}
...
// Compiler generates PostgreSQL SQL from the portable query AST.
...
func (c Compiler) CompileFilter(filter query.Filter) (string, []any, error) {
	if filter == nil {
		return "", nil, nil
	}
	bag := &ArgBag{}
	clause, err := c.compileFilter(filter, bag)
	if err != nil {
		return "", nil, err
	}
	return clause, bag.Args, nil
}
...
func (c Compiler) compileFilter(filter query.Filter, bag *ArgBag) (string, error) {
	...
	case query.EnumEqualsFilter[flowdefinition.Column]:
		if f.Column == flowdefinition.ColStatus {
			return c.Column(f.Column) + " = " + bag.Add(f.Value) + statusCast, nil
		}
		return c.Column(f.Column) + " = " + bag.Add(f.Value), nil
	case query.FlowDefinitionPurposeContains:
		ph := bag.Add(f.Purpose) + purposeElemCast
		return ph + " = ANY(" + c.Column(flowdefinition.ColPurposes) + ")", nil
	default:
		return "", fmt.Errorf("postgres compile: unsupported filter %T", filter)
	}
}
