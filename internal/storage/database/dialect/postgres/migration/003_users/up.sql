...
-- This optimizes joining of tables that have matching partion columns.
...
-- It is partioned by user_id, matching the users table, and therefore optimized to hydrate a user.
...
-- This table is partioned by key: it optimizes looking up a single unique attribute by it's hash,
...
-- We can use less partitions here: the table is really slim and I expect there's ussualy only 1 or 2 unique attributes per user, like username and email.
