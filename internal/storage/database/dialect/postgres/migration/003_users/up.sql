...
/* TODO: template for config
{{ with $n := .UserPartitions -}}
    {{ range $i, $e := int $n | until -}}
CREATE TABLE zitadel_nextgen.users_part_{{ $i }} PARTITION OF zitadel_nextgen.users FOR VALUES WITH (MODULUS {{ $n }}, REMAINDER {{ $i }});
    {{ end }}
{{ end }}
*/
...
/* TODO: template for config
{{ with $n := .UserAttributesPartitions -}}
    {{ range $i, $e := int $n | until -}}
CREATE TABLE zitadel_nextgen.user_attributes_part_{{ $i }} PARTITION OF zitadel_nextgen.user_attributes FOR VALUES WITH (MODULUS {{ $n }}, REMAINDER {{ $i }});
    {{ end }}
{{ end -}}
*/
