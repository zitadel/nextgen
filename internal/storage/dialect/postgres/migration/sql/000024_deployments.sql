...
CREATE TABLE zitadel_nextgen.deployments (
    ...
    -- start, and CreateDeployment's transaction can begin before it waits on
    -- the environment row lock. A deploy that waited would then stamp a time
    -- older than the deploy it waited for, and the newest-first list would
    -- order the current deployment behind the one it replaced.
    ...
    -- If a release is ever deleted directly, this cascade can remove the row
    -- environments.current_deployment_id points at; CreateDeployment treats
    -- that dangling pointer as nothing running, so the next deploy heals it.
