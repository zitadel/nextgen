...
func (ss sessionStatements) ApplyExchange(ctx context.Context, projectID, sessionID string, lastVerifiedChecks map[domain.AuthCheckType]v2session.StoredCheck) error {
	...
	for _, c := range lastVerifiedChecks {
		...
		for _, typ := range v2session.ClassMembers(c.Type) {
			if _, err := ss.client.Exec(ctx, deleteSessionCheckLoserStmt, projectID, sessionID, typ, c.ID); err != nil {
				return wrapError(err)
			}
		}
	}
...
var sessionSchema = database.NewSchema(map[domain.SessionField]database.FieldBinding[domain.Session]{
	...
	domain.SessionFieldHasVerifiedFactors: {
		SQLName:  "EXISTS (SELECT 1 FROM zitadel_nextgen.checks vc WHERE vc.project_id = s.project_id AND vc.session_id = s.id AND vc.last_verified_at IS NOT NULL)",
		Computed: true,
	},
	...
	domain.SessionFieldLifecycleOwnerTeamID: {
		SQLName: `EXISTS (SELECT 1 FROM zitadel_nextgen.users u
