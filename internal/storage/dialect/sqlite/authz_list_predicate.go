...
// Expiry uses a bound now (Unix nanos), matching listAuthzObjectIDs.
func writeAuthzListPredicate(c *statementCompiler, hasWhere *bool, outerResourceIDCol string, f service.AuthzListFilter) {
	...
	c.WriteString(" AND (")
	writeAuthzListBranches(c, f, now)
	c.WriteString("))")
}

func writeAuthzListBranches(c *statementCompiler, f service.AuthzListFilter, now int64) {
