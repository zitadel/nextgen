...
func TestHasher_ValidateEncodedHash(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "out of upper limit, bounds error",
			hash:    "$2a$31$aaaaaaaaaaaaaaaaaaaaaa$aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
			wantErr: ErrBoundsError,
		},
		{
			name:    "invalid hash, invalid error",
			hash:    "$unknown$hash",
