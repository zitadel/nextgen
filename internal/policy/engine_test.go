...
func TestConstraintsListsEveryRule(t *testing.T) {
	...
	want := `{"operation":"user.password.save","rules":{"history":{"history_depth":0},"min_length":{"min_length":15}}}`
	if string(out) != want {
...
func TestValidateInstance(t *testing.T) {
	...
	tests := []struct {
		...
		{"unknown setting", `{"kind":"policy","operation":"user.password.save","config":{"max_length":9}}`, `"max_length" is not defined`},
