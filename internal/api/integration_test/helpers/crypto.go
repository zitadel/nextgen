...
func (h *Harness) EnsureMasterKey(t *testing.T) [32]byte {
	t.Helper()
...
func (h *Harness) EnsureMasterKeyCrypter(t *testing.T) crypto.Crypter {
	t.Helper()
	if h.Crypter == nil {
		h.Crypter = op.NewAES256GCMCrypto(
			h.EnsureMasterKey(t),
			"", // TODO: must be empty to match the master key id for now
		)
	}
	return h.Crypter
}
