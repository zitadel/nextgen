...
func TestDeployments(t *testing.T) {
	...
	t.Run("the record carries resolved ids and the caller identity", func(t *testing.T) {
		...
		message, _ := deployed.Metadata.Message.Get()
	...
	t.Run("promote records the source environment", func(t *testing.T) {
		...
		sourceName, _ := promoted.Metadata.SourceEnvironmentName.Get()
		assert.Equal(t, api.EnvironmentName("dev"), sourceName,
			"the name as it stood, snapshotted for the history")
	})
	...
	t.Run("re-deploying the running release is a no-op", func(t *testing.T) {
		...
		message, _ := reused.Metadata.Message.Get()
		assert.Equal(t, "hotfix for the login outage", message,
			"the answer keeps the message it was created with")
...
func TestDeploymentValidation(t *testing.T) {
	...
	// Shape before existence: the source is invalid for a deploy whatever
	// name it carries, so an unknown one answers the same 400 rather than
	// leaking whether the name resolves.
	t.Run("deploy with an unknown source is still invalid, not not-found", func(t *testing.T) {
		resp := fixture.deploy(t, &api.CreateDeploymentRequest{
			Environment:       "dev",
			ReleaseID:         api.ReleaseID(fixture.releaseID),
			Reason:            api.NewOptDeploymentReason(api.DeploymentReasonDeploy),
			SourceEnvironment: api.NewOptNilEnvironmentName("nosuchenv"),
		})
		expectError(t, resp, http.StatusBadRequest, domain.ErrDeploymentInvalid(nil, nil).Code)
	})
}
