...
func (h *Handler) QueryTeams(ctx context.Context, req *api.QueryTeamsRequest, params api.QueryTeamsParams) (api.QueryTeamsRes, error) {
	ctx, proceed, err := h.requireProjectListAccess(ctx, string(params.ProjectID), teamAccess)
	if err != nil || !proceed {
		return nil, err
	}
	ctx, err = h.withAuthzListFilterIfNeeded(ctx, string(params.ProjectID), domain.ResourceKindTeam, opRead)
	if err != nil {
		return nil, err
	}
