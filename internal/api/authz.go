...
// Project secrets still need project.write and may not act across projects
// without a foothold check (preview stays denied). User principals from a
// Console session skip the secret ceiling: home may differ from target, and
// resolver.Check decides Allow / Forbidden / NotFound (ADR 053 §5).
