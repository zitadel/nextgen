...
func (r *FlowStateMachineRuntime) Process(ctx context.Context, client database.QueryExecutor, def *FlowDefinition, state *FlowState, in FlowSubmitInput) (FlowStepResult, error) {
	...
	if routeOutcome == in.Action {
		pk, err := r.processPasskey(ctx, client, state, currentStep, resolved, def, userSchemaURL, in, actionKind)
		if err != nil {
			return FlowStepResult{}, err
		}
		if pk.halt != nil {
...
func (r *FlowStateMachineRuntime) processPasskey(ctx context.Context, client database.QueryExecutor, state *FlowState, step *FlowDefinitionStep, resolved FlowResolvedFields, def *FlowDefinition, userSchemaURL string, in FlowSubmitInput, actionKind FlowActionKind) (passkeyPhaseResult, error) {
	switch {
