...
// connection is identified by: slug, protocol, subject_claim, and the provider
// coordinates. Those values decide which provider account a stored subject
// belongs to, so changing one would repoint existing identities rather than
// reconfigure them. details names the offending field.
