...
type FlowState struct {
	// ID is the flow handle minted at Start. The handler echoes it
	// back on the path (`/flow/{id}/...`) and verifies it matches the
	// cookie so a tampered path id cannot operate against another
	// browser's cookie.
	ID string
