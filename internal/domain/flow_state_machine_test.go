...
// requiredObjectSchemaContent requires objects that constrain nothing
// about their contents. `billing` is required with no `required` of its
// own; `account` is required and requires `prefs`, which again declares
// nothing. `notes` has the same shape as `billing` but is optional.
const requiredObjectSchemaContent string = `{
	"$schema": "https://json-schema.org/draft/2020-12/schema",
	"type": "object",
	"x-auth-methods": { "password": { "enabled": true } },
	"required": ["email", "billing", "account"],
	"properties": {
		"email": { "type": "string", "format": "email", "x-unique": "team" },
		"billing": {
			"type": "object",
...
func requiredObjectDefinition() *domain.FlowDefinition {
	show := domain.FlowStepCompleteShow
	return &domain.FlowDefinition{
		ProjectID:  testProjectID,
		ID:         "def-required-object",
		UserSchema: defaultSchemaURL,
		Purposes: map[domain.FlowDefinitionPurpose]string{
			domain.FlowDefinitionPurposeRegister: "profile",
		},
		Steps: []domain.FlowDefinitionStep{
			{
				Name:   "profile",
				Fields: []domain.Field{"email", "billing.vat_id", "account.prefs.locale", "notes.text"},
				Actions: []domain.FlowStepAction{
					{Name: domain.FlowActionSubmit, Kind: domain.FlowActionKindSubmit, Primary: true},
				},
				Transitions: map[string]domain.FlowStepTransition{
...
// A required object that declares no `required` of its own is satisfied
// by `{}`, and the leaf covering it resolves optional — so a client that
// omits the leaf (an unselected `select`, or any headless caller) would
// otherwise reach create_user with the object missing.
func TestFlowStateMachine_MaterializesRequiredObjects(t *testing.T) {
	t.Parallel()

	world := func(t *testing.T) *flowTestWorld {
		t.Helper()
		w := newFlowTestWorld(t)
		w.schemaResolver.EXPECT().
			Resolve(gomock.Any(), gomock.Any(), gomock.Any(), defaultSchemaURL, gomock.Any()).
			Return(mustUnmarshal[jsonschema.Schema](t, requiredObjectSchemaContent), nil).
			AnyTimes()
		w.authAttemptService.EXPECT().Start(gomock.Any(), gomock.Any()).Return("att-1", nil)
		w.authAttemptService.EXPECT().
			SubmitIdentifier(gomock.Any(), gomock.Any()).
	...
	start := func(t *testing.T, w *flowTestWorld, def *domain.FlowDefinition) *domain.FlowState {
		t.Helper()
		res, err := w.sm.Start(t.Context(), domain.FlowStartInput{
			Definition:    def,
			Purpose:       domain.FlowDefinitionPurposeRegister,
			Session:       domain.FlowSessionRef{ID: "sess-1", Version: 1},
			UserSchemaURL: defaultSchemaURL,
	...
	t.Run("an omitted leaf still materializes the objects the schema requires", func(t *testing.T) {
		t.Parallel()
		w := world(t)
		def := requiredObjectDefinition()

		res, err := w.sm.Process(t.Context(), def, start(t, w, def), domain.FlowSubmitInput{
			Action: domain.FlowActionSubmit,
			Fields: map[string]any{"email": "alice@example.com"},
		})
		require.NoError(t, err)

		// `notes` is optional, so it stays absent — materializing it would
		// invent a node the schema never asked for.
		assert.Equal(t, map[string]any{
			"email":   "alice@example.com",
			"billing": map[string]any{},
			"account": map[string]any{"prefs": map[string]any{}},
		}, res.State.CollectedData.UserData)

		// The point of materializing: the document create_user hands to
		// the schema now satisfies its `required` list.
		require.NoError(t, mustUnmarshal[jsonschema.Schema](t, requiredObjectSchemaContent).
			Validate(res.State.CollectedData.UserData))
	})

	t.Run("a collected leaf is not replaced by the empty object", func(t *testing.T) {
		t.Parallel()
		w := world(t)
		def := requiredObjectDefinition()
		...
		res, err := w.sm.Process(t.Context(), def, start(t, w, def), domain.FlowSubmitInput{
			...
			Fields: map[string]any{
				"email":                "alice@example.com",
				"billing.vat_id":       "CHE-123",
				"account.prefs.locale": "de-CH",
				"notes.text":           "vip",
			},
		})
		require.NoError(t, err)
		...
		assert.Equal(t, map[string]any{
			"email":   "alice@example.com",
			"billing": map[string]any{"vat_id": "CHE-123"},
			"account": map[string]any{"prefs": map[string]any{"locale": "de-CH"}},
			"notes":   map[string]any{"text": "vip"},
		}, res.State.CollectedData.UserData)
	})
}
