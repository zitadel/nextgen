...
function makeConfig(overrides: Parameters<typeof resolveConfig>[0] = { apiUrl: UPSTREAM }) {
...
function mockResponse(
  body = "{}",
  { status = 200, headers = {} as Record<string, string>, cookies = [] as string[] } = {},
): Response {
...
describe("resolveConfig", () => {
  ...
  it("throws EdgeProxyConfigError for invalid URL", () => {
    expect(() => resolveConfig({ apiUrl: "not-a-url" })).toThrow(EdgeProxyConfigError);
  });

  it("throws EdgeProxyConfigError for non-http/s protocol", () => {
    expect(() => resolveConfig({ apiUrl: "ftp://example.com" })).toThrow(EdgeProxyConfigError);
  });

  it("throws EdgeProxyConfigError for pathPrefix without leading slash", () => {
    expect(() => resolveConfig({ apiUrl: UPSTREAM, pathPrefix: "noslash" })).toThrow(
...
describe("handleProxy", () => {
  ...
  it("strips pathPrefix from upstream path by default", async () => {
    const mock = stubFetch(mockResponse());
    const cfg = makeConfig();
    await handleProxy(new Request("http://edge.local/__nextgen/auth/token"), cfg);
    const [url] = mock.mock.calls[0] ?? [];
  ...
  it("preserves query string", async () => {
    const mock = stubFetch(mockResponse());
    const cfg = makeConfig();
    await handleProxy(new Request("http://edge.local/__nextgen/search?q=hello&page=2"), cfg);
    const [url] = mock.mock.calls[0] ?? [];
  ...
  it("injects x-forwarded-host when absent", async () => {
    ...
    await handleProxy(new Request("http://edge.local/__nextgen/ping"), cfg);
    const [, init] = mock.mock.calls[0] ?? [];
    expect((init!.headers as Headers).get("x-forwarded-host")).toBe("edge.local");
  });
  ...
  it("preserves x-forwarded-host when already present", async () => {
    ...
    await handleProxy(req, cfg);
    const [, init] = mock.mock.calls[0] ?? [];
    expect((init!.headers as Headers).get("x-forwarded-host")).toBe("cdn.example.com");
  });
  ...
  it("falls back to x-real-ip for x-forwarded-for when cf-connecting-ip absent", async () => {
    ...
    await handleProxy(req, cfg);
    const [, init] = mock.mock.calls[0] ?? [];
    expect((init!.headers as Headers).get("x-forwarded-for")).toBe("9.10.11.12");
  });
  ...
  // ── projectSecret injection ───────────────────────────────────────────────

  it("attaches the project secret as a Bearer Authorization header", async () => {
    const mock = stubFetch(mockResponse());
  ...
  it("does not attach Authorization when projectSecret is absent", async () => {
    const mock = stubFetch(mockResponse());
    const cfg = resolveConfig({ apiUrl: UPSTREAM });
    await handleProxy(new Request("http://edge.local/__nextgen/ping"), cfg);
    const [, init] = mock.mock.calls[0] ?? [];
  ...
  it("does not attach body for HEAD", async () => {
    const mock = stubFetch(mockResponse());
    const cfg = makeConfig();
    await handleProxy(new Request("http://edge.local/__nextgen/ping", { method: "HEAD" }), cfg);
    const [, init] = mock.mock.calls[0] ?? [];
    expect(init?.body).toBeUndefined();
  });
  ...
  it("uses redirect: manual so 3xx is passed through", async () => {
    ...
    const cfg = makeConfig();
    const res = await handleProxy(new Request("http://edge.local/__nextgen/redir"), cfg);
