...
import type { VerifyJwtOptions } from '../../runtime/lib/jwt';
...
describe('decodeJwt', () => {
  ...
  it('extracts payload claims', () => {
    const kid = nextKid();
    const token = makeJwt(
      { sub: 'user-1', email: 'a@b.com', exp: ts(3600) },
      kid,
    );
...
describe('verifyJwt', () => {
  ...
  describe('signature verification', () => {
    it('returns the payload for a valid token', async () => {
      const kid = nextKid();
      const token = makeJwt(
        { sub: 'user-42', email: 'u@example.com', exp: ts(3600) },
        kid,
      );
    ...
    it('returns null when the payload is tampered', async () => {
      ...
      const evilPayload = b64url(
        Buffer.from(JSON.stringify({ sub: 'evil', exp: ts(3600) })),
      );
  ...
  describe('alg validation', () => {
    it('accepts a token whose alg is in allowedAlgorithms', async () => {
      ...
      const payload = await verifyJwt(
        token,
        baseOpts({ allowedAlgorithms: ['RS256'] }),
      );
      expect(payload).not.toBeNull();
    });
    ...
    it('rejects a token whose alg is not in allowedAlgorithms', async () => {
      ...
      const payload = await verifyJwt(
        token,
        baseOpts({ allowedAlgorithms: ['ES256'] }),
      );
    ...
    it('accepts any alg when allowedAlgorithms is undefined', async () => {
      ...
      const payload = await verifyJwt(
        token,
        baseOpts({ allowedAlgorithms: undefined }),
      );
      expect(payload).not.toBeNull();
    });
  });
  ...
  describe('typ validation', () => {
    it('accepts a token with typ "JWT"', async () => {
      ...
      expect(
        await verifyJwt(
          token,
          baseOpts({ allowedTokenTypes: ['JWT', 'at+JWT'] }),
        ),
      ).not.toBeNull();
    });
    ...
    it('is case-insensitive', async () => {
      ...
      expect(
        await verifyJwt(token, baseOpts({ allowedTokenTypes: ['JWT'] })),
      ).not.toBeNull();
    });
    ...
    it('skips typ check when allowedTokenTypes is empty', async () => {
      ...
      expect(
        await verifyJwt(token, baseOpts({ allowedTokenTypes: [] })),
      ).not.toBeNull();
    });
  });
  ...
  describe('iss validation', () => {
    ...
    it('rejects a token whose iss does not match issuerUrl', async () => {
      const kid = nextKid();
      const token = makeJwt(
        { sub: 'u', iss: 'http://evil.example.com', exp: ts(3600) },
        kid,
      );
      vi.stubGlobal('fetch', mockJwks(kid));
  ...
  describe('aud validation', () => {
    it('accepts when aud matches the configured audience string', async () => {
      ...
      expect(
        await verifyJwt(token, baseOpts({ audience: 'my-app' })),
      ).not.toBeNull();
    });
    ...
    it('accepts when aud array contains the configured audience', async () => {
      const kid = nextKid();
      const token = makeJwt(
        { sub: 'u', aud: ['my-app', 'other-app'], exp: ts(3600) },
        kid,
      );
      vi.stubGlobal('fetch', mockJwks(kid));
    ...
    it('skips aud check when audience option is not set', async () => {
      ...
      expect(
        await verifyJwt(token, baseOpts({ audience: undefined })),
      ).not.toBeNull();
    });
  });
