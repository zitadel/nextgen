...
1. The published Figma library (file key in `figma-tokens.lock`).
   - Canonical **semantic colours** come from the committed DTCG export under
     repo-root `tokens/`, regenerated by `:sync-export`. Variables are not
...
- **Figma plugin push:** branch `design-tokens/figma-sync`, token path `tokens/`.
  CI triggers on `push` to that branch (`tokens/**` only).
- `:sync-export` (`scripts/sync-from-export.ts`) — reads DTCG JSON in `tokens/`
  (e.g. `primitives.json`) and merges into `figma.tokens.json`. Preserves
  primitives, spacing, radius, layout, and legacy login tokens verbatim for
  keys not present in the export.
...
- `scripts/sync-from-export.ts` + repo-root `tokens/` — offline DTCG sync and
  its committed export provenance. Re-export from Figma and re-run `:sync-export`
  to refresh values.
