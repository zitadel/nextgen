...
void main() {
  ...
  test('exchange posts the handoff token and stores the session token',
      () async {
    final client = SessionClient(buildProject([
      jsonResponse(200, {
        'session': sessionBody(),
  ...
  test('a new exchange supersedes the stored token', () async {
    final client = SessionClient(buildProject([
      jsonResponse(
        200,
        {'session': sessionBody(), 'session_token': 'stok_2'},
      ),
    ]));
  ...
  test('a 401 on exchange explains the credentialed-proxy requirement',
      () async {
    final client = SessionClient(buildProject([
  ...
  test('revoke deletes the session and clears the stored token', () async {
    final client = SessionClient(buildProject([
      ZitadelResponse(status: 204, body: ''),
    ]));
    await project.tokenStore.write('stok_1');
  ...
  test('revoking an already-gone session clears the token and succeeds',
      () async {
    final client = SessionClient(buildProject([
  ...
  test('revoke rethrows unexpected failures but still clears the token',
      () async {
    final client = SessionClient(buildProject([
