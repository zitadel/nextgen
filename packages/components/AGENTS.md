...
The orchestrator calls `@zitadel/api-client` (orval-generated typed fetch
...
`@zitadel/api-client/runtime/base-url`, or via the `api-base` attribute
...
- **Wire shapes** — never declared here; imported from
  `@zitadel/api-client/generated/model` (`CreateFlow201`,
...
`tsdown.config.ts` produces ESM + `.d.mts`, externalises runtime libraries
(`lit`, `liquidjs`, `dompurify`, icons), and bundles the public workspace
helpers (`@zitadel/api-client`, design tokens, shared component styles) so the
published component subpaths do not leak workspace-only CSS or source imports.
